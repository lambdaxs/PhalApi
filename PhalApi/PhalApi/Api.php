...
```
     ...
     * @param $pattern正则规则
     * @param $value判断字符串
     * @param $errorMessage错误信息
     */
    protected function checkParam($pattern, $value, $errorMessage){
        if (!preg_match($pattern,$value)) {
            throw  new PhalApi_Exception_BadRequest($errorMessage);
        }
    }
}

class PhalApi_Domain{
    ...
    {
        if ($name == 'userId'){
            return $this->getUserIdByToken($this->token);
        }
    }
    
    //返回报错信息
    protected  function showErrorMessage($msg,$code=1) {
        DI()->response->setData(null);
        throw new PhalApi_Exception_BadRequest($msg,$code);
    }

    //将token转换成uersId 简化客户端的调用 (使用公共token的时限制权限)
    protected function getUserIdByToken($token)
    {
        //查缓存中是否存在有效的token
        $tokenModel = new Model_Token();

        $userId = $tokenModel->getUserIdInMemcache($token);

        if (empty($userId))
            $this->showErrorMessage("token无效");
